...
class TestIntegration(unittest.TestCase):
    ...
    def test_criacao_interface(self):
        interface = InterfaceVisual()
        interface.coletar_dados_salario()
        interface.coletar_dados_periodo()
        interface.coletar_dados_descontos()

        self.assertIsNotNone(interface.dados_salario)
        self.assertIsNotNone(interface.dados_periodo)
        self.assertIsNotNone(interface.dados_descontos)
    ...
    def test_fluxo_completo(self):
        ...
        # Garantir anos de vigência válidos
        self.calculadora.definir_anos_vigencia()
        self.assertEqual(self.calculadora.anos_vigencia, ['2022', '2023'])  # Validação

        # Inicialização do gráfico
        grafico = Grafico(
            salario_liquido=[3000, 3200, 3400],
            irrf_recolhido=[200, 250, 300],
            anos_vigencia=["2021", "2022", "2023"],  # Anos como strings
            numero_de_graficos=3,
            salario_base=[5000, 5200, 5300],
            aditivos=[1000, 1200, 1100],
            salario_base_de_calculo=[4000, 4200, 4300],
            deducoes=[500, 600, 700],
